...
class BookStoreApi:
    ...
    def post_books(self, data)-> tuple[int,str|json] :
        res1 = self._session.post(url=f"{self._url}", data=data)
        if res1.status_code == 201:
            return res1.status_code , res1.json()
        return res1.status_code , res1.text

    def delete_books(self, userId)-> tuple[int,str|json] :
        res1 = self._session.delete(url=f"{self._url}?UserId={userId}")
        if res1.status_code == 204:
            return res1.status_code, res1.json()
        return res1.status_code, res1.text

    def get_book(self, isbn)-> tuple[int,str|json] :
    ...
    def put_books(self, data, isbn)-> tuple[int,str|json] :
