...
def Pet_fix() -> Pet:
    cat = Pet(id=12, name='danny', category=Category(id=1, name='Cats'), tags=[Tag(id=1, name='tag1')])
    return cat
...
def test_update_name_or_status_by_id(test_status, Pet_fix, pet_Api):
    ...
    LOGGER.info(test.text)
    assert test.status_code == 200 and test.json()["id"] == Pet_fix2.to_json()["id"] and test.json()["name"] == \
           Pet_fix2.to_json()["name"] and test.json()["status"] == Pet_fix2.to_json()["status"]
...
def test_get_inventory(order_fix: Order, store_Api):
    ...
    assert res2.status_code == 200 and res2.json() is not None and res3.json() != res1.json()


def test_post_order(order_fix: Order, store_Api):
    res1 = store_Api.post_order(order_fix.to_json())
    LOGGER.info(res1.text)
...
def test_put_username(user_fix3: User, user_Api):
